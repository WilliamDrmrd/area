...
@Injectable()
export class DiscordService implements ServiceProvider {
  discord: Service | undefined;
  name = 'Discord';
  ...
  async saveService() {
    this.discord = await this.prisma.service.upsert({
  ...
  async saveReactions(): Promise<Map<number, ReactionInterface>> {
    ...
    const id = await this.sendMessage.saveReaction(this.discord.id);
