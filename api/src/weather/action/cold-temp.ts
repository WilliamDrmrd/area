...
@Injectable()
export class ColdTemp extends ActionAbstract implements ActionInterface {
  constructor(prisma: PrismaService) {
    super(prisma, 'Cold-temp', 'When temp get cold (< 10)');
  }
  ...
  async execute(
    ...
    if (nine?.main.temp <= 17 || fifteen?.main.temp <= 17) {
      ...
      return {
        ...
        runReaction: true,
        data: {
          message: `tomorrow gonna be cold, temp: ${nine?.main.temp}C° at 9h and ${fifteen?.main.temp}C° at 15h`,
          object: 'tomorrow gonna be cold',
        },
      };
    }
